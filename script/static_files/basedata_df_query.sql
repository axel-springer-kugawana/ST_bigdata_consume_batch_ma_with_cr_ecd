with BaseDataFirst as (
    ...
    ROW_NUMBER() OVER (PARTITION BY classified_metadata_classifiedid, to_date(classified_metadata_changedate) 
                    ORDER BY classified_metadata_changedate DESC, partitioncreateddate DESC) as baseRank
...
CurrentMonthChanges as (
    SELECT 
        *,
        ROW_NUMBER() OVER (PARTITION BY classified_metaData_classifiedId,cleaned_classified_prices_buy_price_amount 
...
LastPriceBeforeMonth as (
    SELECT 
        *,
        ROW_NUMBER() OVER (PARTITION BY classified_metaData_classifiedId 
...
FROM (
    SELECT 
        *,
        ROW_NUMBER() OVER (PARTITION BY classified_metaData_classifiedId, {price_amount_column} 
