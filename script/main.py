...
def modifyDataJson(glueContext, df, distribution_type) -> DynamicFrame:
    ...
    if distribution_type == "BUY":
        ret_df = (
            ret_df.withColumn(
                "prices_buy_price_amount", F.col("classified_prices_buy_price_amount")
            )
            .withColumn(
            ...
            .withColumn(
                ...
                F.when(
                    F.col("classified_prices_buy_operatingCosts_amount").isNotNull(),
            ...
            .withColumn(
                "prices_buy_serviceCharge_currency", F.col("classified_prices_currency")
            )
        )
    else:  # RENT
        ret_df = (
    ...
    ret_df = (
        ...
        .withColumn(
            ...
            + (
                F.when(
                    F.col("classified_structure_parkingLots_streetParking").isNotNull(),
                    F.col("classified_structure_parkingLots_streetParking"),
                ).otherwise(0)
            )
            + (
                F.when(
                    F.col("classified_structure_parkingLots_doubleGarage").isNotNull(),
                    F.col("classified_structure_parkingLots_doubleGarage"),
                ).otherwise(0)
            )
            + (
                F.when(
                    F.col("classified_structure_parkingLots_garageArea").isNotNull(),
        ...
        .withColumn(
            ...
            F.when(
                F.col("classified_structure_building_kitchen_kitchenType").isNotNull(),
        ...
        .withColumn(
            ...
            .when(
                F.col("classified_energy_energyType_energySource_electric").isNotNull(),
                F.lit("electric"),
            )
            .when(
                F.col(
    ...
    if "subTypes" in ret_df.columns:
        ret_df = Helper.construct_MapType(
            ret_df, key_col="estateType", value_col="subTypes", new_col_name="subTypes"
        )
...
for geoid, data_country, distribution_type, data_source in country_values:
    ...
    if union_df is None:
        ...
        union_df = sparkUnion(
            glueContext,
            unionType="ALL",
            mapping={"source1": union_df, "source2": BaseData_df},
