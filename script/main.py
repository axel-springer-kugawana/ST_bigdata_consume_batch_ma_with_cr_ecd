...
def modifyDataJson(glueContext, df, distribution_type) ->  DynamicFrame:
    ...
    # special columns
    ret_df = ret_df.withColumn("spaces_residential_livingSpace",F.when((F.col("classified_spaces_residential_livingSpace").isNotNull()) & (F.col("classified_spaces_residential_livingSpace") != ""),
                                                                          F.col("classified_spaces_residential_livingSpace"))
                                                                          .otherwise(0.0).cast("float"))\
                    .withColumn("spaces_residential_plotSpace",F.when((F.col("classified_spaces_residential_plotSpace").isNotNull()) & (F.col("classified_spaces_residential_plotSpace") != ""),
