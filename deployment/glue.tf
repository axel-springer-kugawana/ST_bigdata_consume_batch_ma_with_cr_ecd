...
resource "aws_s3_object" "upload-pricechange_df_query" {
  bucket = "${var.bucket}-${var.env}"
  key    = "scripts/pricechange_df_query.sql"
  source = "../../script/static_files/pricechange_df_query.sql"
  etag   = filemd5("../../script/static_files/pricechange_df_query.sql")
...
resource "aws_glue_job" "glue-job" {
  ...
  default_arguments = {
    ...
    "--extra-files"                      = "s3://${var.bucket}-${var.env}/scripts/stadtlandkreise.csv,s3://${var.bucket}-${var.env}/scripts/bundeslaender.csv,s3://${var.bucket}-${var.env}/scripts/attributes_all.txt,s3://${var.bucket}-${var.env}/scripts/classified_cols.txt,s3://${var.bucket}-${var.env}/scripts/basedata_df_query.sql,s3://${var.bucket}-${var.env}/scripts/pricechange_df_query.sql"
    "--conf"                             = "spark.sql.extensions=io.delta.sql.DeltaSparkSessionExtension --conf spark.sql.catalog.spark_catalog=org.apache.spark.sql.delta.catalog.DeltaCatalog"
    "--executor-cores"                   = var.env == "live" ? floor(32 * 1.9) : floor(8 * 1.9) # The value should not exceed 2x the number of vCPUs on the worker type, which is 8 on G.1X, 16 on G.2X, 32 on G.4X and 64 on G.8X
  ...
  number_of_workers = var.env == "live" ? 4 : 2
  max_retries       = 0
}
