...
resource "aws_glue_job" "glue-job" {
  ...
  default_arguments = {
    ...
    "--executor-cores"                   = var.env == "live" ? floor(32 * 1.95) : floor(8 * 1.95) # The value should not exceed 2x the number of vCPUs on the worker type, which is 8 on G.1X, 16 on G.2X, 32 on G.4X and 64 on G.8X
